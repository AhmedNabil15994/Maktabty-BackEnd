...
class ProductRepository
{
    ...
    public function getRelatedProducts($productType, $exceptedIds = [], $order = 'id', $sort = 'desc')
    {
        $products = $this->product->active();
        if ($productType) {
        ...
        return $products->orderBy($order, $sort)->get(['id', 'title']);
    }
    ...
    public function create($request)
    {
        ...
        try {
            if (is_null($request->product_type)) {
                $request->product_type = ProductType::__default;
            }
            $data = [
            ...
            if ($request->product_type == ProductType::Note) {
                $data['educational_institution_id'] = $request->educational_institution_id;
            }
            ...
            if ($request->product_type == ProductType::Product && $request->category_id) {
    ...
    public function filterDataTable($query, $request)
    {
        ...
        if (isset($request['req']['product_type']) && !empty($request['req']['product_type'])) {
            $query->where('product_type', $request['req']['product_type']);
        }
