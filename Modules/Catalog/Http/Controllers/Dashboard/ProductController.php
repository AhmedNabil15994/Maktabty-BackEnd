...
use Modules\Catalog\Transformers\Dashboard\RelatedProductResource;
...
class ProductController extends Controller
{
    ...
    {
        $relatedProducts = $this->product->getRelatedProducts(ProductType::__default);
        return view('catalog::dashboard.products.create', compact('relatedProducts'));
    }
    ...
    public function edit($id)
    {
        ...
        $relatedProducts = $this->product->getRelatedProducts($product->product_type, [$product->id]);
        return view('catalog::dashboard.products.edit', compact('product', "currentVaration", 'relatedProducts'));
    }
    ...
    {
        try {
            $exceptedIds = $request->id ? [$request->id] : [];
            $data = $this->product->getRelatedProducts($request->type, $exceptedIds);
            return response()->json(RelatedProductResource::collection($data));
        } catch (\PDOException$e) {
            return response()->json([false, $e->errorInfo[2]]);
        }
    }
