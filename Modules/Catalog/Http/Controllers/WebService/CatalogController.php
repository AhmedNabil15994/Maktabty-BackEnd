...
use Modules\Catalog\Transformers\WebService\EducationalInstitutionResource;
...
class CatalogController extends WebServiceController
{
    ...
    public function getEducationalInstitutions(Request $request)
    {
        $items = $this->catalog->getEducationalInstitutions($request);

        if ($request->response_type == 'paginated') {
            return $this->responsePagination(EducationalInstitutionResource::collection($items));
        } else {
            return $this->response(EducationalInstitutionResource::collection($items));
        }
