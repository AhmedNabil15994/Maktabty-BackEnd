...
class OrderRepository
{
    ...
    {
        return $this->order->where('from_cashier', 0)->whereHas('orderStatus', function ($query) {
            $query->successOrderStatus();
        })->count();
    }
    ...
    public function getOnlinePendingOrders($minutes)
    {
        $minutes = '-' . $minutes . ' minutes';
        $currentDate = new \DateTime;
        $currentDate->modify($minutes);
